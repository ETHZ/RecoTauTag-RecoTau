...
bool trkPixelHits_cand(const PFCandidate& cand, int cut) 
{
  ...
  if ( track.isNonnull() ) return trkPixelHits(track, cut);
  else return false;
}
...
{
  auto track = getTrackRef(cand);
  if ( track.isNonnull() ) return trkTrackerHits(track, cut);
  else return false;
}
...
{
  auto track = getTrackRef(cand);
  if ( track.isNonnull() ) return trkTransverseImpactParameter(track, pv, cut);
  else return false;
}
...
{
  auto track = getTrackRef(cand);
  if ( track.isNonnull() ) return trkLongitudinalImpactParameter(track, pv, cut);
  else return false;
}
...
{
  auto track = getTrackRef(cand);
  if ( track.isNonnull() ) return minTrackVertexWeight(track, pv, cut);
  else return false;
}
...
{
  auto track = getTrackRef(cand);
  if ( track.isNonnull() ) return trkChi2(track, cut);
  else return false;
}
