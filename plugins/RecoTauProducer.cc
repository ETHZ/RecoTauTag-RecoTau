...
RecoTauProducer::RecoTauProducer(const edm::ParameterSet& pset) 
{
  ...
  for ( VPSet::const_iterator builderPSet = builders.begin();
    ...
    const std::string& pluginName = builderPSet->getParameter<std::string>("name");
    // Build the plugin
    try {
  ...
  for ( VPSet::const_iterator modfierPSet = modfiers.begin();
    ...
    const std::string& pluginName = modfierPSet->getParameter<std::string>("name");
    // Build the plugin
    reco::tau::RecoTauModifierPlugin* plugin = 0;
    try {
      plugin = RecoTauModifierPluginFactory::get()->create(pluginType, *modfierPSet);
    } catch (...) {
...
void RecoTauProducer::produce(edm::Event& evt, const edm::EventSetup& es) 
{
  ...
  evt.getByLabel(chargedHadronSrc_, chargedHadronAssoc);
